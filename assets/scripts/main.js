...
function initializeServiceWorker() {
  ...
  if ("serviceWorker" in navigator) {
    ...
    window.addEventListener("load", async () => {
      ...
      try {
        const regi = await navigator.serviceWorker.register("./sw.js", {
          scope: "/",
        });
    ...
    console.log("? B2+ don't really work if B1 fails");
  }
}
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (const recipeURL of RECIPE_URLS) {
      ...
      try {
        ...
        const fetchedResponse = await fetch(recipeURL);
        ...
        //            "await" again
        const fetchJSON = await fetchedResponse.json();

        // A8. TODO - Add the new recipe to the recipes array
        recipesArray.push(fetchJSON);
